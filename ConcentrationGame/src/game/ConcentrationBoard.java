...
public class ConcentrationBoard {
    ...
    public ConcentrationCard getCard(int row, int col) throws ConcentrationException {
        //TODO YOUR CODE HERE
        if (row > DIM || row < 0)
            throw new ConcentrationException("invalid row value : " + row);
        if (col > DIM || col < 0)
            throw new ConcentrationException("invalid column value : " + col);
    ...
    public CardMatch reveal(int row, int col) throws ConcentrationException {
        ...
        if (row > board.length || col > board.length || row < 0 || col < 0) {
            throw new ConcentrationException(" The coordinates are invalid ");
        } else if (gameOver()) {
            throw new ConcentrationException(" The game is over. ");
        } else if (!board[row][col].isHidden()) {
            throw new ConcentrationException(" The card has already been revealed. ");
        }

        // checks for the revealed card.
        if (revealedCard == null) {
            cardMatch = new CardMatch(board[row][col], revealedCard, false);
            revealedCard = board[row][col];
            revealedCard.reveal();
        } else {
            if (board[row][col].equals(revealedCard)) {
                this.matches += 1;
                cardMatch = new CardMatch(board[row][col], revealedCard, true);
